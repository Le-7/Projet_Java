...
public class Board {
	...
	public boolean isSolvable() {
		...
		if (boardSize % 2 == 1) {
			// Pour un taquin de taille impaire, il est résoluble si le nombre d'inversions
			// est pair
			return inversions % 2 == 0;
		} else {
			// Pour un taquin de taille paire, il est résoluble si :
			// - le nombre d'inversions est impair et la ligne de la case vide (0) est
			// paire,
			// - ou le nombre d'inversions est pair et la ligne de la case vide (0) est
			// impaire.
	...
	public boolean solveWithinTime(long timeLimitMillis, List<String> solution) {
		...
		try {
			...
			for (String move : solution) {
				...
				// Extraire le mouvement (première partie) pour l affichage si quelqun a la
				// force de le faire
